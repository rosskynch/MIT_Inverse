...
*/
  ...
  template <int dim>
  Point<dim> rotate_point(const Point<dim> &input_point,
                          ...
                          const double &angle)
  {
    ...
    // if angle is very close to zero, then nothing to do.
    if (abs(angle) < 1e-15)
    {
      return Point<dim> (input_point);
    }
    Tensor<1,dim> cross_prod;
    cross_product(cross_prod, axis, input_point);
    Point<dim> output_point(
      input_point*cos(angle)
      + cross_prod*sin(angle)
      + axis*(axis*input_point)*(1-cos(angle))
    );
    return output_point;
  }
  template Point<3> rotate_point(const Point<3> &,
                                 const Tensor<1,3> &,
                                 const double &);
  
  template <int dim>
  Tensor<1,dim> rotate_tensor(const Tensor<1, dim> &input_tensor,
                              const Tensor<1, dim> &axis,
                              const double &angle)
  {
    // Rotates a vector (point) about an axis by an angle using
  ...
  template Tensor<1,3> rotate_tensor(const Tensor<1,3> &,
                                     const Tensor<1,3> &,
                                     const double &);
                                       
  
  template <int dim>
  FullMatrix<double> rodrigues_rotation_matrix(const Tensor<1,dim> &axis,
                                               const double &angle)
  {
    // This returns the rotation matrix corresponding to the rotation about the given axis
    // by the given angle.
    if (abs(angle) < 1e-15)
    {
      FullMatrix<double> output (IdentityMatrix(3));
      return output;
    }
    else
    {
      FullMatrix<double> output(3);
      const double omc = 1.0-cos(angle);
      output(0,0) = cos(angle) + axis[0]*axis[0]*omc;
      output(1,0) = axis[2]*sin(angle) + axis[0]*axis[1]*omc;
      output(2,0) = -axis[1]*sin(angle) + axis[0]*axis[2]*omc;
      
      output(0,1) = axis[0]*axis[1]*omc - axis[2]*sin(angle);
      output(1,1) = cos(angle) + axis[1]*axis[1]*omc;
      output(2,1) = axis[0]*sin(angle) + axis[1]*axis[2]*omc;
      
      output(0,2) = axis[1]*sin(angle) + axis[0]*axis[2]*omc;
      output(1,2) = -axis[0]*sin(angle) + axis[1]*axis[2]*omc;
      output(2,2) = cos(angle) + axis[2]*axis[2]*omc;
      
      // Since this is a rotation matrix, then it must be orthogonal(i.e. determinant is +/-1)
      // Further, proper rotation matrices must have determinant +1
      // Check this before returning. If this warning comes up then it's likely the axis of rotation
      // was not found correctly (could be zero axis with angle pi)
      const double tol = 1e-15;
      if (output.determinant() < 0 || output.determinant() > 1.0+tol)
      {
        std::cout << "rodrigues_rotation_matrix:" << std::endl
        << "WARNING: determinant of rotation matrix is not 1. Check the axis and angle of rotation!" << std::endl;
      }
      
      return output;
    }
  }
  template FullMatrix<double> rodrigues_rotation_matrix(const Tensor<1,3> &,
                                                        const double &);
