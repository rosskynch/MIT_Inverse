...
namespace ForwardSolver
{
  ...
  EddyCurrent<dim, DH>::EddyCurrent(const EddyCurrentData &data_in,
                                    DH &dof_handler,
                                          ...
                                          
  {
    ...
    if (p_order > 0 && data.constrain_gradients)
    {
      ...
      for (;cell!=endc; ++cell)
      {
        ...
        if (data.conducting_material[cell->material_id()])
        {
  ...
  template<int dim, class DH>
  void EddyCurrent<dim, DH>::update_eddy_data(const EddyCurrentData &data_in)
  {
    // Update the privately stored data structure.
    data = data_in;
    
    // Update the kappa parameter from the updated data.
    // Note:
    // Kappa = Kappa_re + i*Kappa_im = -omega^2*epr + i*omega*sigma
    // Additionally, we scale the whole equation by mu0,
    param_kappa.resize(2);
    param_kappa[0].resize(data.n_materials);
    param_kappa[1].resize(data.n_materials);
    for (unsigned int i=0; i<data.n_materials; ++i)
    {
      // For eddy current problems, this should always be zero.
      // However, we allow non-zero Re(kappa) for testing purposes.
      param_kappa[0][i] = -data.param_omega*data.param_omega*data.param_epsilon[i]*EquationData::constant_mu0;
      param_kappa[1][i] = data.param_sigma[i]*data.param_omega*EquationData::constant_mu0;
    }
  }
  ...
  void EddyCurrent<dim, DH>::assemble_matrices (const DH &dof_handler)
  {
    ...
    = { {0.0, -data.param_regularisation},
        {-data.param_regularisation, 0.0} };
    ...
    for (; cell!=endc; ++cell)
    {
      ...
      const double current_mur = data.param_mur[cell->material_id()];
      const double mur_inv = 1.0/current_mur;
      const double current_kappa_re = param_kappa[0][cell->material_id()];
      const double current_kappa_im = param_kappa[1][cell->material_id()];
      // for preconditioner:
                                           ...
                                           const EddyCurrentFunction<dim> &boundary_function)
  {
    ...
    for (; cell!=endc; ++cell)
    {
      ...
      if (!boundary_function.zero_rhs())
      {
        ...
        for (unsigned int i=0; i<dofs_per_cell; ++i)
        {
          if (!constraints.is_constrained(local_dof_indices[i]))
          {
            ...
            // is communicated by data.rhs_factor, which
      ...
      if (!boundary_function.zero_curl() && data.neumann_flag)
      {
        // Neumann storage
  ...
  void EddyCurrent<dim, DH>::initialise_solver() // removed:(const bool direct_solver_flag)
  {
    ...
    {
      if (data.direct_flag) // use direct solver (LU factorisation of whole matrix).
      {
  ...
  void EddyCurrent<dim, DH>::solve (Vector<double> &output_solution, unsigned int &n_iterations)
  {
    ...
    {        
      ...
      SolverControl solver_control (system_matrix.m(),
                                    data.solver_tolerance*system_rhs.l2_norm(),
                                    true, true); // Add to see residual history
      
      GrowingVectorMemory<BlockVector<double> > vector_memory;
      SolverGMRES<BlockVector<double> >::AdditionalData gmres_data;
      gmres_data.max_n_tmp_vectors = 250;
      gmres_data.right_preconditioning = data.right_preconditioning;
      
      // Care needs to be taken with the residual monitor.
      gmres_data.use_default_residual = data.right_preconditioning ? true : false;
